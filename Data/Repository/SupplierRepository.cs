...
namespace Data.Repository
{
    public class SupplierRepository : ISupplierRepository
    {
        ...
        {
            var supplier = myContext.Suppliers.Where(s => s.Name.ToLower() == supplierVM.Name.ToLower() || s.Email.ToLower() == supplierVM.Email.ToLower()).FirstOrDefault();
            int result = 0;
            if (supplier == null)
            {
        ...
        {
            var supplier = myContext.Suppliers.FirstOrDefault(s => s.Name.ToLower() == supplierVM.Name.ToLower() || s.Email.ToLower() == supplierVM.Email.ToLower());
            int result = 0;
            if (supplier == null)
            {
